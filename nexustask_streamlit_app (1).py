...
def seed_data():
    ...
    if not c.fetchall():
        tasks = [
            ("Design UI", "Create homepage mockup", "Pending", "bob", "Alpha", "2025-09-10"),
            ("Backend API", "Develop login API", "In Progress", "bob", "Alpha", "2025-09-12"),
...
if st.session_state.user is None:
    ...
    if user['role'] == "admin":
        ...
        tasks = fetch_tasks()
        st.dataframe(tasks)
        fig = px.histogram(tasks, x="status", title="Task Status Distribution")
        ...
        if st.button("Break Down Selected Task"):
            if not tasks.empty:
                task_title = tasks.iloc[0]["title"]
                st.write(gemini_breakdown(task_title, api_key))
